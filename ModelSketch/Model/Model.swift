...
class Model {
    ...
    init() {
        ...
        let nodeI = self.constructionGraph.createNode()
        let nodeJ = self.constructionGraph.createNode()
        self.constructionGraph.connect(nodeA: nodeE, nodeB: nodeF)
        self.constructionGraph.connect(nodeA: nodeG, nodeB: nodeH)
        self.constructionGraph.connect(nodeA: nodeI, nodeB: nodeJ)
        ...
        self.constructionGraph.add(relationship: FollowPencilRelationship(node: nodeG, cgPoint: CGPoint(x: 500.0, y: 550.0)))
        self.constructionGraph.add(relationship: FollowPencilRelationship(node: nodeH, cgPoint: CGPoint(x: 600.0, y: 550.0)))
        self.constructionGraph.add(relationship: FollowPencilRelationship(node: nodeI, cgPoint: CGPoint(x: 500.0, y: 600.0)))
        self.constructionGraph.add(relationship: FollowPencilRelationship(node: nodeJ, cgPoint: CGPoint(x: 600.0, y: 600.0)))
        self.update()
        ...
        let ijDistance = DistanceRelationship(nodeIn: nodeI, nodeOut: nodeJ)
        let jiDistance = DistanceRelationship(nodeIn: nodeJ, nodeOut: nodeI)
        ...
        self.constructionGraph.add(relationship: ijDistance)
        self.constructionGraph.add(relationship: jiDistance)
        efDistance.setEqualRelationship(ghDistance)
        feDistance.setEqualRelationship(hgDistance)
        ghDistance.setEqualRelationship(ijDistance)
        hgDistance.setEqualRelationship(jiDistance)
